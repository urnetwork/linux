...
// rotationLambda(dl) first adds dl to the longitude, then rotationPhiGamma(dp,
// 0) takes the unit vector
...
// = rotated x, so the three helpers below are the whole pipeline with no
// inverse trig.
...
double RotateTowardViewer(float lonDeg, float latDeg, float rotLambdaDeg, float rotPhiDeg) {
  ...
  return std::cos(lambda) * std::cos(phi) * std::cos(deltaPhi) - std::sin(phi) * std::sin(deltaPhi);
}
...
// range(-180, 180, 90), major parallels range(0, 90 - eps, 360) (the equator
// only), minor meridians range(-180, 180, 10) filtered to abs(lon % 90) > eps,
// and minor parallels range(-80, 80 + eps, 10) filtered to abs(lat % 360) > eps.
...
std::vector<double> SampleSpan(double start, double end) {
  ...
  std::vector<double> out(static_cast<size_t>(steps) + 1);
  for (int i = 0; i <= steps; ++i) {
    out[static_cast<size_t>(i)] = (i == steps) ? end : start + i * kGraticulePrecision;
  }
  return out;
}

std::vector<float> Meridian(double lon, double latStart, double latEnd) {
  const std::vector<double> lats = SampleSpan(latStart, latEnd);
  std::vector<float> line(lats.size() * 2);
...
std::vector<float> Parallel(double lat) {
  const std::vector<double> lons = SampleSpan(-180.0, 180.0);
  std::vector<float> line(lons.size() * 2);
...
std::vector<std::vector<float>> BuildGraticule() {
  ...
  for (double lon = -180.0; lon < 180.0; lon += 10.0) {
    if (std::fabs(std::fmod(lon, 90.0)) > kGraticuleEpsilon) {
  ...
  for (double lat = -80.0; lat <= 80.0 + kGraticuleEpsilon; lat += 10.0) {
    if (std::fabs(lat) > kGraticuleEpsilon) {
      lines.push_back(Parallel(lat));
    }
  }
  return lines;
}
...
GlobeProjection Project(float lonDeg, float latDeg, float rotLambdaDeg, float rotPhiDeg,
  ...
  if (towardViewer < 0.0) return GlobeProjection{GlobePoint{}, false};
  const double right = RotateRight(lonDeg, latDeg, rotLambdaDeg);
  const double up = RotateUp(lonDeg, latDeg, rotLambdaDeg, rotPhiDeg);
  return GlobeProjection{GlobePoint{static_cast<float>(kCenter + scale * right),
...
GlobePoint ProjectClamped(float lonDeg, float latDeg, float rotLambdaDeg, float rotPhiDeg,
  ...
  if (length < 1e-9) {
    return GlobePoint{kCenter + scale, kCenter};
  }
...
GlobeRotation RotationCentering(float lonDeg, float latDeg) {
  return GlobeRotation{-lonDeg, -latDeg};
}
...
float DragDegreesPerVirtualPx(float scale) {
  return static_cast<float>(static_cast<double>(kVirtualSize) / scale /
                            (3.0 * 3.14159265358979323846));
}

const std::vector<std::vector<float>>& Graticule() {
  // computed once; rotation is applied at draw time
  static const std::vector<std::vector<float>> kLines = BuildGraticule();
  return kLines;
}

WheelStep ResolveWheelStep(float travel, float threshold) {
  if (threshold <= 0.0f) {
    return WheelStep{0, 0.0f};
  }
...
GlobePoint ToVirtual(float x, float y, float canvasWidth, float canvasHeight) {
  const float unit = UnitFor(canvasWidth, canvasHeight);
  if (unit <= 0.0f) {
    return GlobePoint{kCenter, kCenter};
  }
