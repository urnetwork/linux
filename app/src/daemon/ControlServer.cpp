...
std::string ControlServer::PolicyPath() {
  ...
  return ctl::kPolkitPolicyPath;
}

bool ControlServer::PolkitPolicyPresent() {
  return ::access(PolicyPath().c_str(), F_OK) == 0;
}
...
void ControlServer::Dispatch(uint64_t connId, const nlohmann::json& request, ReplyFn reply) {
  ...
  RequireAuth(connId, actionId, interactive, id,
              [this, connId, id, verb, isLogTail, requestCopy, reply](bool ok,
...
void ControlServer::DispatchAuthorized(uint64_t connId, int64_t id, ctl::Verb verb,
  ...
  try {
    ...
    // world-connectable.
    if (isLogTail) {
      if (conn->peer.uid != 0 && TunnelOwnedByOtherUid(conn)) {
    ...
    switch (verb) {
      case ctl::Verb::StartTunnel:
        reply(HandleStartTunnel(conn, id, request));
        return;

      case ctl::Verb::StopTunnel: {
        nlohmann::json denied;
        bool crossUid = false;
        if (!CheckTunnelOwner(conn, id, &denied, &crossUid)) {
          reply(std::move(denied));
          return;
        }
...
bool ControlServer::CheckTunnelOwner(Connection* conn, int64_t id, nlohmann::json* denied,
  ...
  if (cross && authMode_ == AuthMode::Polkit) {
    // Getting here means RequireAuth already carried kActionTakeOverTunnel —
    // auth_admin_keep in all three slots, so an administrator password was
    // supplied even at the console. Displacing the other user's session is
    // exactly what was authorized, so the per-connection check below must not
    // then refuse it.
    return true;
  }
...
nlohmann::json ControlServer::HandleStartTunnel(Connection* conn, int64_t id,
  ...
  nlohmann::json denied;
  bool crossUid = false;
  if (!CheckTunnelOwner(conn, id, &denied, &crossUid)) return denied;
