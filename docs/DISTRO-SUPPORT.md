...
| H4 | nftables features: `inet` family, `type route hook output`, and **named counters** | `BuildNftRuleset` **[M-src]** | `nft -f` rejects the whole transaction → no IPv6/DNS floor, no witness, no start | `nft --check -f -` against the generated ruleset (parses and evaluates, **never commits**) |
| H5 | When nft socket-cgroup matching is available, nft resolves its cgroup path **at load time**, so the path must exist | `CgroupInstallable` guards it — an absent path fails the *entire* transaction, taking the kill switch with it **[M-src]** | Guarded; listed so nobody removes the guard | compare the `0::/path` in `/proc/self/cgroup` with `/sys/fs/cgroup/path` |
...
| S7 | nft supports **`socket cgroupv2 level N "path"`** (`CONFIG_NFT_SOCKET` plus compatible userspace) | The daemon probes the exact live path with `nft --check`. If the BPF socket mark is proven, a normal floorless tunnel omits the unavailable cgroup belt and keeps the mark witness. A kill-switch floor or helper-DNS reconnect refuses rather than weakening crash/helper-cgroup safety. If the BPF marker is not proven, every tunnel refuses. Docker Desktop LinuxKit is the measured mark-only case. | `sudo urnetworkd --selftest-egress`, then inspect the startup log for `cgroup_match=0`; `zgrep NFT_SOCKET /proc/config.gz` distinguishes the measured kernel omission from a missing path. |
